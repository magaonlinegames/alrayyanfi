...
function LOGIN_USER(){
    ...
    if (email != '') {
        ...
        db.collection("BANKS").where("logs", "==", logs)
        .get()
        .then((querySnapshot) => {
...
function BANK_AUTH(IP){
    ...
    docRef.get().then((doc) => {
        if (doc.exists) {
            ...
            // open account
            var docRef = db.collection("BANKS").doc(accountGD);
            docRef.get().then((doc) => {
                if (doc.exists) {
        ...
        querySnapshot.forEach((doc) => {
            ...
            if (doc.data().which_account == user_account) {
                console.log('BBC-777');
                 ...
                 setTimeout(() => {
                 $('.transfer-info-txt').text('We are verying this transaction...');    
                 }, 15000);
...
function load_account(whichAcc){
    ...
    .onSnapshot((doc) => {
        ...
        if (doc.data().account_refresh == '1') {
            db.collection("BANKS").doc(whichAcc).update({
                account_refresh: '0'
            })
            .then(() => {
        ...
        .then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
                ...
                if (doc.data().account == whichacc) {
                    ...
                    console.log(doc.id, " => ", doc.data());
                    if (doc.data().transaction_status == 'pending_debit') {
                        var szData = '<li class="row">';
                        szData += '<span class="col s2">';
                        szData += '<i class="material-icons left grey-text">alarm</i>';
                        szData += '</span>';
                        szData += '<span class="col s7 tdets">';
                        szData += '<span class="transaction_details">';
                        szData += doc.data().description;
                        szData += '</span>';
                        szData += '</span>';
                        szData += '<span class="col s3  dateAndAmountBX">';
                        szData += '<span class="transaction_amount_new">';
                        szData += doc.data().amount;
                        szData += '</span>';
                        szData += '<span class="transaction_date_new">';
                        szData += doc.data().date;
                        szData += '</span>';
                        szData += '</span>';
                        szData += '</li>'  ;        
                        szData += '<hr></hr>';     
                        ...
                        $('#recent_transaction_bx').append(szData);
                    } else if (doc.data().transaction_status == 'failed_transaction') {
                        var szData = '<li class="row">';
                        szData += '<span class="col s2">';
                        szData += '<i class="material-icons left red-text">close</i>';
                        szData += '</span>';
                        szData += '<span class="col s7 tdets">';
                        szData += '<span class="transaction_details">';
                        szData += doc.data().description;
                        ...
                        $('#recent_transaction_bx').append(szData);
                            ...
                            var szData = '<li class="row">';
                            szData += '<span class="col s2">';
                            szData += '<i class="material-icons left">check</i>';
                            szData += '</span>';
                            szData += '<span class="col s7 tdets">';
                            szData += '<span class="transaction_details">';
                            szData += doc.data().description;
